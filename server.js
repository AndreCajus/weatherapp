...
app.post('/api', (req, res) => {  
  ...
  database.insert(Object.assign({'method' : req.method, 'status' : 'SUCCESS', 'rpath' : req.route.path, 'body': [data]}));
  res.json(data);
});
...
app.get('/forecast/daily/:city', async (req, res) => {
  ...
  try {
    ...
    if (city_name.split(",", 1) == city) {
      ...
      database.insert(Object.assign( {'method' : req.method, 'status' : 'SUCCESS', 'rpath' : req.route.path}, timestamp));
    }else {
      res.status(400).json({status: 400, message: "Locality does not have related weather information!"});
      database.insert(Object.assign( {'method' : req.method, 'status' : 'FAILURE', 'rpath' : req.route.path}, timestamp));
    }
  } catch (err) {
